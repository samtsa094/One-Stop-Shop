    ...
    if form_id == "login_form":
        ...
        for shop in shops:
            if request.form.get("email") == shop["email"]:
                if sha256_crypt.verify(request.form.get("password"), shop["password"]):
...
def add_stock(id):
    ...
    mongo.db.Products.update_one(
        {"_id": ObjectId(id)},
        {"$inc": {"quantity": int(request.form.get("quantity"))}}
    )
    return redirect("/owner_shop")
...
@app.route("/view_shop/", methods=["GET"])
def view_shop(email):
...
@app.route("/add_cart_home/", methods=["POST"])
...
@app.route("/add_cart_shop//", methods=["POST"])
